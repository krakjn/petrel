...
class ByteArrayType : public TopicDataType {
  public:
    ByteArrayType() { set_name("ByteArray"); }

    bool
    serialize(const void *const data, eprosima::fastdds::rtps::SerializedPayload_t &payload,
...
class PetrelDataReaderListener : public DataReaderListener {
  public:
    PetrelDataReaderListener(
        ...
        const std::string &topic_name, TypeSupport type)
        : callback_(callback), topic_name_(topic_name), type_(type) {}

    void on_data_available(DataReader *reader) override {
        if (!callback_ || !type_ || !reader)
            return;
...
class Publisher::Impl {
  public:
    Impl() : participant_(nullptr), fastdds_publisher_(nullptr) {}
    ...
    bool publish(const std::string &topic, const std::vector<uint8_t> &data) {
        ...
        if (it == writers_.end()) {
            ...
            if (!fastdds_topic) {
                // Create topic
                fastdds_topic =
                    participant_->create_topic(normalized_topic, "ByteArray", TOPIC_QOS_DEFAULT);
            ...
            // Create writer
            DataWriter *writer =
                fastdds_publisher_->create_datawriter(fastdds_topic, DATAWRITER_QOS_DEFAULT);
...
class Subscriber::Impl {
  public:
    Impl() : participant_(nullptr), fastdds_subscriber_(nullptr) {}
    ...
    bool subscribe(const std::string &topic) {
        ...
        // Create listener
        PetrelDataReaderListener *listener =
            new PetrelDataReaderListener(callback_, normalized_topic, type_support_);

        // Create reader
        DataReader *reader =
            fastdds_subscriber_->create_datareader(fastdds_topic, DATAREADER_QOS_DEFAULT, listener);
...
bool Subscriber::subscribe(const std::string &topic) { return pimpl_->subscribe(topic); }

void Subscriber::stop() { pimpl_->stop(); }
